...
//        Localizations.localeOf(context).languageCode);
  ...
  static int getUpdateStatus(String version) {
    String locVersion =
        (Platform.isAndroid ? AppConfig.androidVersion : AppConfig.iosVersion);
    int remote = int.tryParse(version.replaceAll('.', '')) ?? 0;
    int loc = int.tryParse(locVersion.replaceAll('.', '')) ?? 0;
    if (remote <= loc) {
      return 0;
    } else {
